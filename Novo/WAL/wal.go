...
func (wal *WAL) convert(data []byte) []Entry { //konvertuje niz bajtova u niz entrya
	...
	for i < uint64(len(data)) {
		...
		keySize := binary.LittleEndian.Uint64(data[i+TOMBSTONE_SIZE : i+KEY_SIZE])
		value_size := binary.LittleEndian.Uint64(data[i+KEY_SIZE : i+VALUE_SIZE])
		keyData := string(data[i+VALUE_SIZE : i+VALUE_SIZE+keySize])
		val := data[i+VALUE_SIZE+keySize : i+VALUE_SIZE+keySize+value_size]
		...
		// valculate new index
		i = i + VALUE_SIZE + keySize + value_size //pomeri za duzinu celog reda
	}
	return rez
}
...
func NewWAL(path string, duration time.Duration, lowMark int) (*WAL, error) { //konstruktor
	cache := lru.NewLRU()
	return &WAL{
		path:      path,
		segments:  []*Segment{},
...
func (wal *WAL) cacheit(index int64, value []byte) { //stavlja u kes
	key := strconv.Itoa(int(index))
	wal.cache.Set(key, value)
...
func Put(wal *WAL, mem *Mem.Memtable, key string, value []byte) bool {
	c := config.Config{}
	c.ReadConfigFile()
	segment, _ := wal.GetLastSegment()
	if segment.Size() > int64(c.GetSizeSegments()) {
		_, err := wal.NewSegment()
		if err != nil {
			return false
		}
	}
	err := wal.Set([]*T{&T{key, value, false}})
	if err != nil {
		return false
	}
	mem.Add(key, value)
...
func Delete(wal *WAL, mem *Mem.Memtable, key string) {
	var data []byte
	segment, _ := wal.GetLastSegment()
	if segment.Size()+int64(len(data)) > 300 {
		_, _ = wal.NewSegment()
	}
	err := wal.Set([]*T{&T{key, []byte{}, true}})
	if err != nil {
		return
	}
	mem.Delete(key)
