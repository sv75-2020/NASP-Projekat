...
func (m *Memtable) Delete(key string){
	...
	m.data.delete(e.key)			// tombstonujemo po kljucu
...
func main (){
	memtable := New(10, 1000)
	memtable.Add("drzava", []byte("srbija"))
	memtable.Add("selo", []byte("klek"))
	memtable.Add("reka", []byte("dunav"))

	fmt.Println(memtable.Get("drzava"))
	fmt.Println(memtable.Get("selo"))
	fmt.Println(memtable.Get("reka"))

	fmt.Println("SIZE", memtable.size)
